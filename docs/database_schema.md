...
|---------------------|---------------|----------------|--------------------------|
| id                  | bigint        | PRIMARY KEY    | 主键                     |
| symbol              | string        | NOT NULL, UNIQUE, INDEX | 资产代码 (如 AAPL) |
...
| captured_at         | datetime      | NOT NULL, INDEX | 快照时间                 |
| price               | decimal(15,2) | NOT NULL       | 资产价格                 |
| change_percent      | decimal(8,4)  |                | 价格涨跌幅 (%)           |
| volume              | decimal(20,0) |                | 成交量                   |
| created_at          | datetime      | NOT NULL       | 创建时间                 |
| updated_at          | datetime      | NOT NULL       | 更新时间                 |

**索引**:
- `index_asset_snapshots_on_asset_id_and_captured_at (asset_id, captured_at)` (复合唯一索引)
- `index_asset_snapshots_on_asset_id`
...
### 2.9 strategy_factor_weights (策略因子权重表)
